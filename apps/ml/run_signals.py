...
def _impacto_estimado(tipo: str, features: dict, avg_monthly: float) -> float:
    ...
    if tipo == "REPOSICION":
        return avg_monthly
    # DETERIORO_SOSTENIDO — usar drop_amount si disponible, sino estimación mensual
    drop_amount = features.get("drop_amount")
    if drop_amount is not None:
        return float(drop_amount) * 3
...
def main() -> dict[str, int]:
    ...
    print("2.6. Guardando estado global de clientes para gráficas del Frontend...")
    ...
    # Avg monthly value per client×subfamily — proxy para impacto_estimado commodity
